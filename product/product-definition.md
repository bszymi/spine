...
**Partial-merge handling.** Suppose `api-gateway`'s merge succeeds but `payments-service` has a conflict. The run transitions to `partially-merged`. The primary repo records `api-gateway: merged`, `payments-service: conflict`. While the run is `partially-merged`, all run branches stay preserved — including `api-gateway`'s already-merged branch — so an operator can inspect the merged work alongside the failed branch when resolving the conflict. The task remains open. An operator resolves the conflict on the failed branch, retries the merge, and once every repo has merged the run advances to fully completed and the run branches are cleaned up.
