...
**Partial-merge handling.** Suppose `api-gateway`'s merge succeeds but `payments-service` hits a merge conflict. The run transitions to `partially-merged`. The primary repo records the per-repo outcomes — `api-gateway: merged`; `payments-service: failed` with `failure_class: merge_conflict` — using the canonical `RepositoryMergeStatus` schema. While the run is `partially-merged`, all run branches stay preserved — including `api-gateway`'s already-merged branch — so an operator can inspect the merged work alongside the failed branch when resolving the conflict. The task remains open. An operator resolves the conflict on the failed branch, retries the merge, and once every repo has merged the run advances to fully completed and the run branches are cleaned up.
