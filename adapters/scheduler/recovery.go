...
// RunCommitRetry triggers a commit-retry tick when a commit-retry function
// is configured. Public wrapper around retryCommittingRuns added by
// TASK-004 extraction so service/internal/scheduler can drive per-workspace
// retries without reaching into adapter internals.
func (s *Scheduler) RunCommitRetry(ctx context.Context) {
	if s.commitRetryFn == nil {
		return
	}
