...
4. Two terminal exits exist:
   - **Resolve and resume** — the operator clears the failing code repo's outcome by flipping its `RepositoryMergeStatus` away from `failed`. Spine exposes two supported recovery APIs on the Orchestrator that produce the correct status transitions and audit ledger commits:
     ...
     A `skipped` outcome is the right target when the work has been retracted entirely. The status `merged` is reserved for Spine-driven merges (Spine fills `merge_commit_sha` and `merged_at` itself); operator recovery should NOT bypass the recovery APIs to write `merged` directly into `runtime.repository_merge_outcomes`, even though the SQL and `RepositoryMergeOutcome.Validate` checks do not block it — direct DB edits skip the audit trail and the per-run ledger commit. The scheduler's periodic sweep (`Scheduler.retryCommittingRuns`) gates on `codeRepoOutcomesAllowResume`: it re-issues `git.retry_partial_merge` only once no non-primary outcome is still `failed`. Fixing the code repo's branch tip without updating the outcome row leaves the run parked, because the gate stays closed. The terminal-skip guard in `internal/engine/multi_repo_merge.go` prevents double-merging repos that already succeeded.
