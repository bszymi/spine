...
4. Two terminal exits exist:
   - **Resolve and resume** — the operator clears the failing code repo's outcome by flipping its `RepositoryMergeStatus` away from `failed`. Spine exposes two supported recovery APIs on the Orchestrator that produce the correct status transitions and audit ledger commits:
     - `Orchestrator.RetryRepositoryMerge(ctx, runID, repoID, reason)` flips the outcome to `pending` and writes an `EventRunRepositoryMergeRetryRequested` event so the next scheduler tick re-attempts the merge.
     - `Orchestrator.ResolveRepositoryMergeExternally(ctx, runID, repoID, reason, targetCommitSHA)` flips the outcome to `resolved-externally` for cases where the conflict was resolved out of band (the operator merged the branch through a different channel, or decided the work is no longer required and should be marked complete).
     These two APIs are the only operator-driven paths that clear a `failed` outcome with audit. `merged` is reserved for Spine-driven merges (Spine fills `merge_commit_sha` and `merged_at` itself), and `skipped` is set internally by the per-repo merge loop when a repository was in `AffectedRepositories` but the merge step was skipped by workflow rules — there is no operator API that writes `skipped` after the fact. Direct edits to `runtime.repository_merge_outcomes` skip the audit trail and the per-run ledger commit and should not be treated as a supported recovery path. The scheduler's periodic sweep (`Scheduler.retryCommittingRuns`) gates on `codeRepoOutcomesAllowResume`: it re-issues `git.retry_partial_merge` only once no non-primary outcome is still `failed`. Fixing the code repo's branch tip without updating the outcome row leaves the run parked, because the gate stays closed. The terminal-skip guard in `internal/engine/multi_repo_merge.go` prevents double-merging repos that already succeeded.
