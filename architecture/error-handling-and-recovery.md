...
4. Two terminal exits exist:
   - **Resolve and resume** — the operator clears the failing code repo's outcome by flipping its `RepositoryMergeStatus` away from `failed` (e.g., to `merged` after a manual merge, `resolved-externally` for an out-of-band fix, `skipped` if the work is no longer needed) or by mutating the underlying code repo's branch tip so the next merge attempt succeeds. The scheduler's periodic sweep (`Scheduler.retryCommittingRuns`) automatically re-issues `git.retry_partial_merge` once no non-primary outcome is still `failed`; the run re-enters `committing`. The terminal-skip guard in `internal/engine/multi_repo_merge.go` prevents double-merging repos that already succeeded.
