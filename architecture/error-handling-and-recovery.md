...
4. Two terminal exits exist:
   - **Resolve and resume** — the operator clears the failing code repo's outcome by flipping its `RepositoryMergeStatus` away from `failed`. Acceptable target statuses are `pending` (to request another automated merge attempt — `Orchestrator.RetryRepositoryMerge` produces this transition), `resolved-externally` (when the operator has merged the branch out of band or otherwise considers the conflict resolved without Spine performing the merge), or `skipped` (when the work is no longer needed). The status `merged` is reserved for Spine-driven merges that record `merge_commit_sha` and `merged_at` — operators MUST NOT flip a row directly to `merged`; the SQL check constraint and `RepositoryMergeOutcome.Validate` enforce this. The scheduler's periodic sweep (`Scheduler.retryCommittingRuns`) gates on `codeRepoOutcomesAllowResume`: it re-issues `git.retry_partial_merge` only once no non-primary outcome is still `failed`. Fixing the code repo's branch tip without updating the outcome row leaves the run parked, because the gate stays closed. The terminal-skip guard in `internal/engine/multi_repo_merge.go` prevents double-merging repos that already succeeded.
