...
| `400 Bad Request` with invalid clone URL | `clone_url` does not parse, or scheme is outside the allowlist | Use `https://`, `ssh://`, `git+ssh://`, or SCP-like (`user@host:path`) form. `http://`, `git://`, `file://`, `ext::`, and any other scheme are rejected (cleartext, no-auth, local-FS exfiltration, or smart-protocol command execution risks); leading whitespace and leading `-` are also rejected |
