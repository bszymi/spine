...
| `PUT` succeeds but pushes still fail with `auth_failed` | The new value at `secret-store://workspaces/<ws>/git` is wrong, OR a layer below the binding is caching the old value | Verify the secret backend has the new credential at the canonical ref. If a cached value is suspected, call `Invalidate` on the secret-client (§5.4) and retry. Do NOT change `credentials_ref` to a different scheme as a remedy — the canonical ref is the only supported form, and rewriting it would convert `auth_failed` into `credentials_unavailable` |
...
| Inactive repository in a task's `repositories:` list | Run-start fails: catalog-only RE-001 rule rejects the unknown ID; the engine precondition / step-routing checks reject inactive bindings | RE-001 lives in `internal/validation/rules_repository.go`. Engine-side inactive-repo handling is in `internal/repository/registry.go` (`ErrRepositoryInactive` from `Registry.Lookup`). Product context: `/product/multi-repository-workspaces.md` §3.2 |
